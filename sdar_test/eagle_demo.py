"""
SGLang EAGLE Speculative Decoding Demo
=======================================
...
接受率通常在 70-90%，可实现 2-3× 的吞吐提升。

【重要】关于模型选择
---------------------
EAGLE 加速效果依赖两个条件：
  1. Draft 模型必须是官方训练的（yuhuili / SafeAILab），社区模型质量不稳定
  2. 目标模型不能太小：7B 在 H100 上 baseline 已达 160+ tok/s，EAGLE 开销
     会超过收益；13B 及以上的模型（baseline 60-100 tok/s）才有明显加速

官方验证可用的模型对（单张 H100 80GB）
-----------------------------------------
  推荐：
    Target : meta-llama/Llama-2-13b-chat-hf
    Draft  : yuhuili/EAGLE-llama2-chat-13B
    预期加速: 2.3-3×
  ...
  其他官方支持的模型对（需更多显存或多卡）：
    Target : meta-llama/Llama-2-70b-chat-hf  (需 2×H100 或 INT4)
    Draft  : yuhuili/EAGLE-llama2-chat-70B
    Target : meta-llama/Meta-Llama-3-70B-Instruct  (需 2×H100 或 INT4)
    Draft  : yuhuili/EAGLE-LLaMA3-Instruct-70B

EAGLE 核心参数说明
------------------
  speculative_algorithm        : "EAGLE"（固定）
  speculative_draft_model_path : draft 模型路径（必须与 target 版本严格匹配）
  speculative_num_steps        : 每次 draft 的前向步数，推荐 3-5
  speculative_eagle_topk       : 每步保留 top-k 候选，推荐 4-8
  speculative_num_draft_tokens : 从候选树中最终选出的草稿 token 数（推荐 16-64）

使用方法（服务器模式）
----------------------
Step 1: 在另一个终端启动 EAGLE 服务器

  python -m sglang.launch_server \\
      --model-path meta-llama/Llama-2-13b-chat-hf \\
      --speculative-algorithm EAGLE \\
      --speculative-draft-model-path yuhuili/EAGLE-llama2-chat-13B \\
...
Step 3: 运行本 demo
  ...
  # 只测 EAGLE
  python eagle_demo.py --port 30000

  # 和基线对比（推荐）
  python eagle_demo.py --port 30000 --baseline-port 30001 --compare

  # 使用离线 Engine API（无需启动服务器）
  python eagle_demo.py --offline
"""
...
# 推荐模型对：官方训练的 EAGLE draft，单张 H100 80GB 可运行，预期加速 2.3-3×
DEFAULT_TARGET_MODEL = "meta-llama/Llama-2-13b-chat-hf"
DEFAULT_DRAFT_MODEL = "yuhuili/EAGLE-llama2-chat-13B"

# EAGLE 推荐参数
EAGLE_NUM_STEPS = 5          # draft 步数（13B 以上推荐 5）
